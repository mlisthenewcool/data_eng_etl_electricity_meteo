...
- **ETL** (Python/Polars/DuckDB) : Sources distantes → Landing → Bronze → Silver (fichiers .parquet)
- **Load** (psycopg/COPY) : Silver .parquet → PostgreSQL
- **ELT** (dbt-core) : Silver (PostgreSQL) → Gold (PostgreSQL)
...
parquet). Le traitement **avant** chargement est justifié car PostgreSQL ne sait pas
...
Une fois les données normalisées dans PostgreSQL, les transformations gold (jointures,
...
spatiales et des optimisations du query planner PostgreSQL.
...
| Silver .parquet → Silver PG | psycopg, `COPY` | Load : chargement bulk dans PostgreSQL |
