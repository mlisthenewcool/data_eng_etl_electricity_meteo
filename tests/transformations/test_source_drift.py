...
_EXPECTED = frozenset({"col_a", "col_b", "col_c"})
_DATASET = "test_dataset"


class TestValidateSourceColumns:
    def test_exact_match_passes(self) -> None:
        lf = pl.DataFrame({"col_a": [1], "col_b": [2], "col_c": [3]}).lazy()
        validate_source_columns(lf, expected_columns=_EXPECTED, dataset_name=_DATASET)

    def test_added_columns_raises(self) -> None:
        lf = pl.DataFrame({"col_a": [1], "col_b": [2], "col_c": [3], "col_new": [4]}).lazy()
    ...
    def test_removed_columns_raises(self) -> None:
        lf = pl.DataFrame({"col_a": [1], "col_b": [2]}).lazy()
        with pytest.raises(SourceSchemaDriftError) as exc_info:
            validate_source_columns(lf, expected_columns=_EXPECTED, dataset_name=_DATASET)
    ...
    def test_added_and_removed_columns_raises(self) -> None:
        lf = pl.DataFrame({"col_a": [1], "col_x": [2], "col_y": [3]}).lazy()
        with pytest.raises(SourceSchemaDriftError) as exc_info:
            validate_source_columns(lf, expected_columns=_EXPECTED, dataset_name=_DATASET)
    ...
    def test_column_order_does_not_matter(self) -> None:
        lf = pl.DataFrame({"col_c": [1], "col_a": [2], "col_b": [3]}).lazy()
        validate_source_columns(lf, expected_columns=_EXPECTED, dataset_name=_DATASET)

    def test_accepts_dataframe(self) -> None:
        df = pl.DataFrame({"col_a": [1], "col_b": [2], "col_c": [3]})
        validate_source_columns(df, expected_columns=_EXPECTED, dataset_name=_DATASET)
