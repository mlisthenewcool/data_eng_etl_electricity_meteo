...
- **File Assets** (``get_silver_file_asset``): ``file://`` URIs pointing to
  silver Parquet files on disk. Produced by ingestion DAGs.
- **Postgres Assets** (``get_silver_pg_asset``): ``postgres://`` URIs
...
def get_silver_file_asset(dataset_name: str) -> Asset:
    ...
    Landing and bronze are internal pipeline stages with no downstream
    consumers. Gold datasets live in Postgres (via dbt).
...
def get_silver_pg_asset(dataset_name: str) -> Asset:
    ...
    project Postgres database. They serve as outlets for load DAGs
